...
def run_doctor() -> str:
    ...
    # config files
    cfg = USER_APP_DIR / "config" / "app_config.yaml"
    sop = USER_APP_DIR / "sops" / "fbw_a380x_gate_to_gate.yaml"
    lvar = USER_APP_DIR / "config" / "lvar_map_template.yaml"
    for p in [cfg, sop, lvar]:
    ...
    # SimConnect python wrapper (optional)
    try:
    ...
    lines.append("1) FBW A380X + WASimCommander installieren")
    lines.append("2) Doctor erneut starten")
    lines.append("3) LVar/Event-Mapping ausfüllen")
    return "\n".join(lines)
